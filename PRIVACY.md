...
The source-built iPhone companion requests Apple Music access to read recently played songs. It stores reconciled activity locally and asks the user which historical plays to submit before sending selected scrobbles directly to Last.fm. PresenceFM does not upload Apple Music history to a PresenceFM server. iOS may suspend background work, so the companion does not claim continuous playback observation.

On iPhone, Last.fm credentials and session tokens are stored in the device Keychain. Optional cross-device CloudKit coordination is disabled in the default local build and requires a separately configured entitlement-enabled build.
