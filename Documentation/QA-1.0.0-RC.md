...
- [x] At final candidate source and packaging commit `ead5201`, debug `swift test` passes on arm64 macOS 27.0 (26A5388g): 97 tests across 17 suites, including configurable provider priority and concrete reorder-action labels, metadata-free verification export, rejected-scrobble correction, Private Mode intent actions, locally advancing progress with throttled accessibility values, responsive minimum-size contracts, duplicate observable-status suppression, bounded oldest-first health-history retention, and demo-history cleanup that preserves real listens. The earlier optimized Private Mode expiration check also passed 10 repeated runs.
- [x] Address Sanitizer and Thread Sanitizer test runs pass with no reported memory or data-race issue.
- [x] At final candidate source and packaging commit `ead5201`, the release app builds, receives an Apple Development signature, and passes strict bundle verification as 1.0.0 build 1, including discoverable metadata for all three App Intents. The 4,689,708-byte DMG passes `hdiutil verify` with SHA-256 `e5e5a19fa2e5c10720c9448048b3698182cc96ee862eec77e573a9046ee4ddd7`.
- [x] Pull-request CI at head `ead5201` ([run 30727734145](https://github.com/CoolColby23/PresenceFM/actions/runs/30727734145)) passes with pinned Xcode 26.0: 97 Swift tests plus coverage, patch hygiene, strict formatting of the new files, website verification, app packaging, bundle verification, DMG creation, and `hdiutil verify`.
...
- [x] The verified 1.0.0 build 1 bundle from final candidate source and packaging commit `ead5201` was installed in `/Applications` on the arm64 macOS 27.0 test machine after final package verification; executable SHA-256 `52b0a4c95244674fb7b07c83e4d2cb4ffd6e40c6ea8f3eb1594e4377a3d83327` matched the packaged source bundle and the app remained running after a normal launch. The prior installed bundle is recoverable at `/tmp/presencefm-install.IcGKPi/PresenceFM.app` for this test session.
...
- [x] Local cleanup removed all 76 records identified by the reserved `presencefm-demo-` persistent-ID prefix, removed no queued scrobbles, preserved all 418 then-current real history records, and left the SQLite store with a successful integrity check. The pre-cleanup store is recoverable from the app's manual backup directory. Candidate `ead5201` now performs this scoped cleanup whenever Demo Mode ends.
...
- The clean five-minute accelerated Demo Mode sample at `28c0e25` passes the
  ...
  normal account-backed playback; poll latency, launch timing, and the four-hour
